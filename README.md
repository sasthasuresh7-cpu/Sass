 
GLOBAL
- OpenCorporates: https://opencorporates.com/
- GLEIF LEI Search: https://search.gleif.org/
- Global Database: https://www.globaldatabase.com/

INDIA
- MCA Portal: https://www.mca.gov.in/ (MCA Services → Master Data)

USA
- All state business searches: https://www.secstates.com/
- Alt list: https://www.llcuniversity.com/50-secretary-of-state-sos-business-entity-search/
- SEC EDGAR: https://www.sec.gov/edgar/search/

UK
- Companies House search: https://find-and-update.company-information.service.gov.uk/

HONG KONG
- ICRIS: https://www.icris.cr.gov.hk/

CHINA (Mainland)
- National Enterprise Credit Info: http://www.gsxt.gov.cn/

SINGAPORE
- ACRA BizFile+: https://www.bizfile.gov.sg/

ISRAEL
- Corporations Authority: https://ica.justice.gov.il/

THAILAND
- DBD data warehouse: https://datawarehouse.dbd.go.th/


1. Focal Entity & Counterparty Profile

The focal entity, CALPERS WWW/SCR, is located and banking in the United States. Publicly available information identifies CALPERS as a pension and retirement fund administrator that provides retirement and health-related benefits primarily for public employees and their families.

For the review period from 19-Aug-2024 to 19-Sep-2024, the focal entity conducted 100% interpersonal transactions, amounting to USD 1,381,644.77, across three different dates. The transactions were primarily routed as round-dollar payments.

The counterparty (EN-HR Trans-Ben/Rec/Orig/Send DIF) is also based in the United States. Available information suggests the counterparty is associated with HR-related disbursements, employee benefits, and record-keeping transactions, which aligns with the focal’s functional profile.


---

2. Transaction Analysis

The focal entity has conducted a series of high-value, round-dollar transactions. Such round-dollar payments can occasionally trigger alerts; however, based on the business nature of CALPERS and the characteristics of the transactions, the activity appears consistent with:

periodic retirement/benefits disbursements,

inter-account fund allocations, and

internal reconciliation or payroll-related processes.


A review of historic patterns indicates that the focal entity frequently conducts fund movements between related accounts for operational, administrative, and record-keeping purposes. This supports the legitimacy of the current activity.

The DS alert for Recurring Originator/Beneficiary (ORB) events was triggered for the current set of transactions. However, both parties are based in the USA (low-risk jurisdiction), and the financial behaviour is in line with a high-capacity institution managing recurring inflows and outflows for benefit distribution. Thus, the recurring nature of these payments does not raise concern.

No unusual structuring, layering behaviour, third-party involvement, or deviations from expected transaction patterns were observed.


---

3. Red Flag Check & Adverse Media Review

A full red-flag assessment and adverse media check were completed.

No negative news, compliance issues, sanctions exposure, or financial misconduct could be linked to either the focal or the counterparty.

No additional concerns surfaced during the extended search.



---

4. Conclusion & Recommendation

The associated alert events were reviewed thoroughly in the context of the entities involved, their business profiles, and the transactional behaviour.

The activity does not exhibit unusual or suspicious characteristics.

The round-dollar nature and recurring or



The focal entity QUESERÍA DOS LAGUNAS S.A. de C.V., incorporated in Mexico, was identified through public domain sources as a company engaged in the manufacturing, processing, and distribution of dairy products, primarily cheese and related food items. The Spanish term “Quesería” directly refers to a cheese or dairy-processing facility, which aligns with the company’s operational nature as a food and dairy manufacturer. The counterparty QUESERIA DOS LAGUNAS carries the same naming structure, indicating that both entities belong to the same corporate group. Based on this and the observed payment pattern, the relationship is classified as inter-company.

A total of 12 outward remittances were initiated by the focal from 24 March 2025 to 03 November 2025, amounting to USD 1,726,000. All transactions were routed to the counterparty entity under the same corporate name, further reinforcing the intra-group nature of the transfers. The OBI reference “PAGO DE INTERESES O COMISIONES”, which translates to “Payment of interest or commissions/fees,” provides a clear explanation of the payment purpose. This indicates that the remittances relate to financial obligations such as inter-company interest settlements, fee payments, or internal accounting adjustments commonly observed within multinational or multi-entity corporate structures.

Despite the relatively high cumulative transaction value, the focal entity reports an annual revenue of approximately USD 78.31 million, suggesting that the remitted amounts are not disproportionate and fall within a reasonable operational scale for a company of this size. The transaction trend also shows no rapid circularity, irregular spikes, or deviation from normal inter-company financial activity.

Mexico is acknowledged as a high-risk jurisdiction, primarily due to elevated financial crime exposure and regulatory vulnerabilities. However, in this case, the following mitigating factors significantly reduce the overall risk:

The focal entity is conclusively identified through public domain sources.

The counterparty shares the same corporate name, confirming a legitimate inter-company relationship.

100% of the transactions fall under the inter-company category.

A clear purpose of payment (“interest or commission payments”) is established.

The transaction amounts are proportionate when compared to the company’s annual revenue.

No adverse media, sanctions concerns, or inconsistencies were observed.


Based on the above, while the jurisdictional risk remains inherently elevated, the overall transactional risk is considered mitigated due to clear entity identification, legitimate business purpose, proportionality of transaction amounts, and a fully established inter-company relationship. No suspicious indicators were identified that would warrant further escalation.
